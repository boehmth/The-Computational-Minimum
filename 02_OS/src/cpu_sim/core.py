...
class RAM(Element):
    """RAM mit variabler Groesse.

    Bei size = 16 (Standard): 4x4-Grid, address ist eine 4-Bit-Zahl.
    Bei size = 256: 16 Segmente à 16 Zellen, `self.address` ist die
    volle 8-Bit-Adresse (SEG << 4 | offset). Angezeigt wird nur das
    Segment, das durch (address >> 4) angesprochen wird, plus eine
    kleine Uebersicht ueber die Belegung der anderen Segmente.
    """
    def __init__(self, x, y, size=16):
        title = f"RAM 16x4" if size == 16 else f"RAM {size//16}×16x4"
        # Bei segmentiertem RAM 1 Extra-Zeile fuer Segment-Uebersicht
        height = 7 if size == 16 else 8
        super().__init__(x, y, width=42, height=height, name=title)
        self.size = size
        self.cells = [0] * size
        self.address = 0
    ...
    def _segment(self):
        return self._addr() >> 4 if self.segmented else 0

    def _offset(self):
        return self._addr() & 0xF

    def read_from_bus(self, bus_value):
        if self.gate == Gate.IN:
            self.cells[self._addr()] = bus_value & 0b1111

    def write_to_bus(self):
        if self.gate == Gate.OUT:
            return self.cells[self._addr()] & 0b1111
        return None

    def render(self):
        seg  = self._segment()
        off  = self._offset()
        base = seg * 16    # Anfang des aktuell gezeigten Segments

        rows = []

        # Segment-Uebersicht (nur bei segmentiertem RAM)
        if self.segmented:
            marks = []
            for s in range(16):
                # ist irgendeine Zelle in diesem Segment != 0?
                used = any(self.cells[s*16 + i] for i in range(16))
                if s == seg:
                    marks.append(c(f"{s:X}", "byellow"))
                elif used:
                    marks.append(c(f"{s:X}", "white"))
                else:
                    marks.append(c(f"{s:X}", "gray"))
            rows.append(c(" seg: ", "gray") + " ".join(marks))

        # 4x4-Grid des aktuellen Segments
        header = "    " + " ".join(c(f" {i:X} ", "gray") for i in range(4))
        rows.append(header)
        for row in range(4):
            cells = []
            for col in range(4):
                i = row * 4 + col       # offset im Segment
                gi = base + i           # globaler Index
                v = self.cells[gi]
                if i == off and self.gate != Gate.NONE:
        ...
        if self.segmented:
            rows.append(c(f"  seg={seg:X}  off={off:X}   "
                          f"value = {self.cells[self._addr()]:X}",
                          "gray"))
        else:
            rows.append(c(f"  addr = {off:X}   "
                          f"value = {self.cells[self._addr()]:X}",
                          "gray"))
...
# ------------------------------------------------------------
# SegmentRegister (fuer segmentierten Speicher).
# Ist ein normales Register, aber didaktisch besonders: darf nur
# vom OS gesetzt werden (nicht durch normalen Mikrocode). Deshalb
# hat es kein Bus-Gate.
# ------------------------------------------------------------
class SegmentRegister(Element):
    def __init__(self, x, y):
        super().__init__(x, y, width=BOX_W, height=4, name="SEG")
...
# ------------------------------------------------------------
# BasePointer (fuer segmentierten Programmspeicher).
# BP zeigt auf einen von 16 "Slots" à 16 Instruktionen im PROG-ROM.
# Die effektive Instruktionsadresse ist (BP << 4) | PC.
#
# BP wird NICHT ueber den Bus geschrieben — der einzige Weg ist:
#   * SETBP-Opcode  -> BP := BX ; PC := 0     (OS uebergibt Kontrolle)
#   * HLT-Opcode    -> BP := 0 ; PC := 0      (Kontrolle zurueck ans OS)
# Damit ist BP effektiv Ring-0, ohne dass wir eine formale MMU brauchen.
# ------------------------------------------------------------
class BasePointer(Element):
    def __init__(self, x, y):
        super().__init__(x, y, width=BOX_W, height=4, name="BP")

    def render(self):
        return self.framed(_reg_content(
            fmt_value(self.value, "bmagenta"),
            "prog base (OS)"))
...
class ControlUnit(Element):
    ...
    def next_control_word(self):
        if self.step == 0:
            ...
            # Effektive Instruktionsadresse:
            #   * mit Base-Pointer:   addr = (BP << 4) | PC
            #   * ohne:               addr = PC
            # Der Programmspeicher enthaelt bis zu 256 Zellen (16 Slots
            # à 16 Instruktionen). Wenn das Programm kuerzer ist, wird
            # jenseits davon Opcode 0 (=HLT) gelesen -> Trap zurueck
            # ans OS.
            pc = self.cpu.pc.value
            if self.cpu.bp is not None:
                addr = (self.cpu.bp.value << 4) | (pc & 0xF)
            else:
                addr = pc

            if 0 <= addr < len(self.cpu.program):
                opcode, operand = self.cpu.program[addr]
            else:
                # Ausserhalb des geladenen Programms: leerer Speicher
                # ist per Konvention HLT (Opcode 0). Das gibt die
                # Kontrolle zurueck ans OS, so wie auf einem echten
                # 6502 BRK-Trap ausgeloest wuerde.
                opcode, operand = "HLT", 0
            ...
            if self.cpu.ram is not None:
                # Falls die CPU segmentierten RAM + SEG-Register hat,
                # ergibt die physische Adresse = SEG*16 + operand.
                if self.cpu.seg is not None and self.cpu.ram.segmented:
                    self.cpu.ram.address = (self.cpu.seg.value << 4) | (operand & 0xF)
                else:
...
class CPU:
    ...
    def __init__(self, config, program):
        ...
        self.seg = config.get("seg")   # SegmentRegister, optional
        self.bp  = config.get("bp")    # BasePointer, optional
        ...
        self.carry = False
        self.halted = False
        self.yielded = False           # gesetzt durch YIELD-Opcode
    ...
    def apply_actions(self, signals):
        ...
        # (in tick()). Hier nur die "Post-Bus"-Effekte:
        #   * CE     -> PC += 1
        #   * HLT    -> je nach Modus: entweder wirklich stoppen (klassisch)
        #               oder BP=0, PC=0 (Batch-OS: zurueck ans OS)
        #   * SETBP  -> BP := BX ; PC := 0 (OS verlaesst sich selbst)
        #   * YIELD  -> flag setzen (kooperatives Multitasking)
        if "CE" in signals and self.pc is not None:
            self.pc.tick()

        # SETBP hat Vorrang: BP <- BX, PC <- 0 (und *kein* PC++).
        # Der PC-Tick wurde zwar schon in CE ausgefuehrt (PC=1), aber
        # wir ueberschreiben ihn unten. Das ist wie ein bedingter Jump.
        if "SETBP" in signals and self.bp is not None:
            self.bp.value = self.tmp.value & 0xF   # BX -> BP
            if self.pc is not None:
                self.pc.value = 0

        if "HLT" in signals:
            if self.bp is not None and self.bp.value != 0:
                # Batch-OS-Semantik: HLT im User-Programm gibt die
                # Kontrolle zurueck ans OS. Das OS-Programm liegt bei
                # BP=0.
                self.bp.value = 0
                if self.pc is not None:
                    self.pc.value = 0
                # NICHT halted setzen -- wir wollen weiterlaufen.
            else:
                # BP=0 (OS macht selbst HLT) oder kein BP-Konzept:
                # echter Stopp.
                self.halted = True

        if "YIELD" in signals:
            # Setzt ein Flag, das der OS-Runner ausserhalb der Simulation
            # abfragt, um einen Context-Switch zu machen. Fuer die CPU
            # selbst ist YIELD wie ein NOP + "CU merkt sich, dass es
            # abgeben will".
            self.yielded = True
